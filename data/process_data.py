...
def clean_data(df):
    ...
    print('Duplicates before cleaning: {}'.format(df.duplicated(subset='id', keep='first').sum()))
    df.drop_duplicates(subset ='id', keep = 'first', inplace = True)
    print('Duplicates after cleaning: {}'.format(df.duplicated(subset='id', keep='first').sum()))
    return df
...
def save_data(df, database_filename):
    ...
    df.to_sql('messsages', engine, index=False)
